defmodule Mix.Tasks.Dependents.Tree.Adaptor do
  alias Mix.Tasks.Ver.Get
  ...
  defp tuple_to_map({[dep1, dep2, dep3, dep4], index}, app, count, ranks) do
    %{
      rank: ranks[app],
      chunk: index,
      ver: "../#{app}" |> Get.ver() |> zap_dup(index),
      hex: "../#{app}" |> Get.hex?() |> zap_dup(index),
  ...
  defp zap_dup(value, index) when is_boolean(value) do
    value = if value, do: "Y", else: "n"
    zap_dup(value, index)
  end
